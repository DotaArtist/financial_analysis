...
import os
import jieba
jieba.load_userdict("./my_word.txt")
...
class TrieNode(object):
    ...
    def __init__(self, node, data=None, PMI_limit=5):
        """
        初始函数，data为外部词频数据集
        :param node:
        :param data:
...
def load_data(filename, stopwords):
    ...
    with open(filename, 'r') as f:
        for line in f:
            word_list = [x.lower() for x in jieba.cut(line.strip(), cut_all=False) if x not in stopwords]
